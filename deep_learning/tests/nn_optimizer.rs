...
#[test]
fn opt_msdg_model_test() ->  Result<(),Box<dyn std::error::Error>> {
	let mut rng = Pcg64::from_entropy();
	let mut nn = NeuralNetwork::<f64>::new();
	let mut mlp_model = nn.create_mlp_model("MLP1", &[20,10,1], MLPActivator::Sigmoid);
	...
	let mut optimizer =
		NNOptimizer::<f64>::new(Optimizer::MomentumSDG(MomentumSDG::new(0.2,0.9)),
								&mlp_model);
...

	{
		...
		let render_backend = BitMapBackend::new("mlp_msdg_optimizer.png", (800,600)).into_drawing_area();
		render_backend.fill(&WHITE)?;
		let mut data_points_chart_builder = ChartBuilder::on(&render_backend)
			.caption("simple neural network (mlp,msdg)", ("sans-serif", 50).into_font())
			.margin(10)
			.x_label_area_size(30)
			.y_label_area_size(30)
			.build_cartesian_2d(0.0f32..1.0f32, -2.0f32..2.0f32)?;
