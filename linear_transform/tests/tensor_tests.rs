...
fn tensor_selector_test() {
	...
	let part_of_selector = &selector_table[0..10];
	let st = t.selector(part_of_selector);

	for (i,&j) in (0..10).zip(part_of_selector.iter()) {
		assert_eq!(st.subtensor(i), t.subtensor(j));
	}
