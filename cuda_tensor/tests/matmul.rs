...
fn matmul(x:&[f32], y:&[f32], (m,l,n):(usize,usize,usize))
	  -> Vec<f32> {
...
fn tensor_matmul() -> Result<(),CUDAError> {
	...
	let tensor_x_src:Vec<f32> = vec![ 1.0, 1.0, 1.0, 1.0,
					  2.0, 2.0, 2.0, 2.0,
					  1.0, 2.0, 1.0, 2.0,
					  3.0, 2.0, 1.0, 2.0 ];
	let tensor_y_src:Vec<f32> = vec![ 2.0, 3.0,
					  1.0, 2.0,
					  1.0, 1.0,
					  5.0, 2.0 ];
	for i in 0..M {
	...
	let tensor_x_src:Vec<f32> = (0..M*L).map(|_| uniform_dist.sample(&mut rng) ).collect::<Vec<f32>>();
	let tensor_y_src:Vec<f32> = (0..L*N).map(|_| uniform_dist.sample(&mut rng) ).collect::<Vec<f32>>();
